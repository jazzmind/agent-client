 ...
 * Ingest API Client
 * 
 * Client for interacting with the Busibox Ingest API for file uploads,
...
export async function listKnowledgeBases(token?: string): Promise<KnowledgeBase[]> {
  const response = await fetch(`${INGEST_API_URL}/knowledge-bases`, {
    headers: getIngestApiHeaders(token),
  });
  return handleResponse(response);
}
...
export async function getKnowledgeBase(id: string, token?: string): Promise<KnowledgeBase> {
  const response = await fetch(`${INGEST_API_URL}/knowledge-bases/${id}`, {
    headers: getIngestApiHeaders(token),
  });
  return handleResponse(response);
}
...
export async function getConversationKnowledgeBase(
  ...
  const response = await fetch(
    `${INGEST_API_URL}/knowledge-bases?conversation_id=${conversationId}`,
    {
      headers: getIngestApiHeaders(token),
    }
  );
...
export async function listDocuments(
  ...
  const response = await fetch(
    `${INGEST_API_URL}/knowledge-bases/${knowledgeBaseId}/documents`,
    {
      headers: getIngestApiHeaders(token),
    }
  );
...
export async function getDocument(id: string, token?: string): Promise<Document> {
  const response = await fetch(`${INGEST_API_URL}/documents/${id}`, {
    headers: getIngestApiHeaders(token),
  });
  return handleResponse(response);
}
